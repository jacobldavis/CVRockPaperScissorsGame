...
while start:
    ...
    if not gameState:
        start_text = gameFont.render("Click anywhere to start!", True, color1)
    ...
    if gameState:
        ...
        if current_time_int > 0:
            countdown_text = gameFont.render(f'Shoot!  {current_time_int}', True, color1)
        ...
        if current_time_int == 0:
            ...
            if gameCount == current:
                ...
                if cpuChoice == playerChoice:
                    outcome = "Draw!"
                elif cpuChoice == "Scissors" and playerChoice == "Rock" or cpuChoice == "Rock" and playerChoice == "Paper" or cpuChoice == "Paper" and playerChoice == "Scissors":
                    outcome = "P1 Wins!"
                    player_score += 1
                else:
                    outcome = "CPU Wins!"
        ...
        if current_time_int < 0:
            outcomeText = gameFont.render(outcome, True, color1)
            outcomeRect = outcomeText.get_rect(center=(width/2, 460))
            window.blit(outcomeText, outcomeRect)

            restartText = gameFont.render("Click to restart!", True, color2)
            restartRect = restartText.get_rect(center=(width/2, 60))
            window.blit(restartText, restartRect)
    # Text
    playerText = gameFont.render(playerChoice, True, color2)
    playerRect = playerText.get_rect(center=(80, 460))
    window.blit(playerText, playerRect)

    cpuText = gameFont.render(cpuChoice, True, color2)
    cpuRect = cpuText.get_rect(center=(545, 460))
    window.blit(cpuText, cpuRect)

    title = gameFont.render('Rock, Paper, Scissors, Shoot!', True, color1)
    window.blit(title, (80, 10))
    ...
    window.blit(cpuScoreText, (520, 40))
